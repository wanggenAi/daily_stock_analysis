...
| Candidate Lifecycle 持续研究记忆 | **ACTIVE** | active=120 / dormant=6 / archived-invalidated=0 / events=10056 / focus=6 |
...
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`35957479519`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`35957479519`
- 触发来源：`JEV_ORCHESTRATOR_35957187257`
- 计算执行：**SUCCESS**
- 运行状态：**COMPLETED**
...
- 同轮补证据尝试：**2**；取得证据：**10**；推进硬门槛：**1**。
- 尚未解决硬门槛：**12**。
- 未决原因摘要：涉及 3 只；门槛分布：long_term_demand×3、predictability×3、earnings_authenticity×2、financial_safety×2、moat×2；Top原因：OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET×3、SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT×2、SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT×2、INSUFFICIENT_MULTI_YEAR_MOAT_EVIDENCE×2、CYCLICAL_RESOURCE_REQUIRES_EXPLICIT_CYCLE_RESILIENCE_EVIDENCE×1；样例：000426[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]；002612[long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET、moat:INSUFFICIENT_MULTI_YEAR_MOAT_EVIDENCE、predictability:EARNINGS_STABILITY_THRESHOLD_NOT_MET]；600661[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]
- 请求但未进入本次研究工件：**无**。
- 上一次完整终态 run：`35957479519`；执行 **SUCCESS**；研究终态 **EVIDENCE_EXHAUSTED**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`35957479519`；当前 Lambda：`35957479519`。
- 请求：**4**；研究 BUY：**1**；研究 WAIT_PRICE：**0**；研究 RESEARCH_GAP：**3**；研究 REJECT：**0**。
- 高吸引力但证据不足、优先补证：002612 朗姿股份(quant=76.7501；暂不投入新增资金；等待补齐：predictability、long_term_demand、moat)；600661 昂立教育(quant=67.8299；暂不投入新增资金；等待补齐：predictability、long_term_demand、moat、financial_safety、earnings_authenticity)
