...
|---|---:|---:|---:|---|---|---|---|---|
| 国电南瑞 600406 | 22.23 | 17.46 | -3.87 | REDUCE_25 | **维持减仓25%目标；本轮无新增减仓/退出信号；目标减50股，当前可执行0股（手数约束；禁止向上取整）** | HIGH | ACTIVE/seen=185 | DEEP_REVIEW_PARTIAL |
| 润贝航科 001316 | 28.90 | 49.37 | 12.25 | HOLD_REVIEW | **持有观察** | LOW | ACTIVE/seen=187 | DEEP_REVIEW_PARTIAL |
| 中国平安 601318 | 54.46 | 83.07 | -2.69 | HOLD | **继续持有** | MEDIUM | ACTIVE/seen=112 | DEEP_REVIEW_PARTIAL |
| 洛阳钼业 603993 | 17.81 | 28.13 | -4.37 | HOLD | **继续持有；历史分批加仓授权已消费，本轮新增可执行0股** | HIGH | ACTIVE/seen=154 | DEEP_REVIEW_PARTIAL |

### 每只持仓的决策链

- **国电南瑞 600406**：现价 22.23 / 价值中枢 17.46（价/值 1.27；价值区间 12.84–26.09；区位 **UPPER_VALUE**）；估值信心 **HIGH**；Formal **REDUCE_25**；Deep **PASS 0 / FAIL 0 / UNKNOWN 5**；Lifecycle **ACTIVE / seen=185**；原因码：`V31_IMMEDIATE_VALUATION_SELL;REDUCE_25;SELL_RATIONALE_STABLE_VALUE_PRICE_OVEREXTENSION`。
- **润贝航科 001316**：现价 28.90 / 价值中枢 49.37（价/值 0.59；价值区间 21.64–70.29；区位 **FAIR_VALUE**）；估值信心 **LOW**；Formal **HOLD_REVIEW**；Deep **PASS 2 / FAIL 0 / UNKNOWN 3**；Lifecycle **ACTIVE / seen=187**；原因码：`VALUATION_CONFIDENCE_LOW;REALISTIC_GROWTH_UNSTABLE`。
- **中国平安 601318**：现价 54.46 / 价值中枢 83.07（价/值 0.66；价值区间 未形成完整区间；区位 **UNKNOWN**）；估值信心 **MEDIUM**；Formal **HOLD**；Deep **PASS 0 / FAIL 0 / UNKNOWN 5**；Lifecycle **ACTIVE / seen=112**；原因码：`INSURER_EVIDENCE_VALID;NO_ACTION_THRESHOLD;price_to_neutral=0.656<1.00`。
- **洛阳钼业 603993**：现价 17.81 / 价值中枢 28.13（价/值 0.63；价值区间 18.03–43.07；区位 **BELOW_VALUE**）；估值信心 **HIGH**；Formal **HOLD**；Deep **PASS 4 / FAIL 0 / UNKNOWN 1**；Lifecycle **ACTIVE / seen=154**；原因码：`FUNDAMENTALS_INTACT;NO_ACTION_THRESHOLD`。
...
| Deep 五类硬门槛 + 官方证据 | **ACTIVE** | requested=6 / verified-pass=208 / unresolved=21 |
| Deep Provenance 证据审计 | **ACTIVE** | audit=True / run=35823608137 / unverified-pass=0 |
...
| Deep Research Terminal | **ACTIVE** | BUY=0 / WAIT=0 / GAP=6 / REJECT=0 |
...
- 国电南瑞 600406：ACTIVE / tier=PENDING / 历史被系统重新看见 185 次。
- 润贝航科 001316：ACTIVE / tier=PENDING / 历史被系统重新看见 187 次。
- 中国平安 601318：ACTIVE / tier=PENDING / 历史被系统重新看见 112 次。
- 洛阳钼业 603993：ACTIVE / tier=PENDING / 历史被系统重新看见 154 次。

## 自动深算运行状态

- 当前运行状态来源：**TERMINAL_STATUS**
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`35823336693`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`35823336693`
- 触发来源：`JEV_ORCHESTRATOR_35823072401`
- 计算执行：**SUCCESS**
- 运行状态：**COMPLETED**
- 研究过程终态：**EVIDENCE_EXHAUSTED**
- 请求深算：**6**；已处理：**6**；完整：**0**；证据穷尽：**6**。
- Workset profile：总数 **500**；请求代码已落 profile **6**；handoff 未完成 **0**；覆盖可审计：**True**；完整覆盖：**True**。
- 同轮补证据尝试：**2**；取得证据：**15**；推进硬门槛：**1**。
- 尚未解决硬门槛：**21**。
- 未决原因摘要：涉及 6 只；门槛分布：long_term_demand×5、moat×5、predictability×5、earnings_authenticity×3、financial_safety×3；Top原因：OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET×5、INSUFFICIENT_CONSECUTIVE_COMPLETE_FISCAL_YEARS×5、SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT×3、SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT×3、DURABLE_MOAT_CORROBORATION_THRESHOLD_NOT_MET×3；样例：000096[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]；001316[long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET、moat:DURABLE_MOAT_CORROBORATION_THRESHOLD_NOT_MET、predictability:INSUFFICIENT_CONSECUTIVE_COMPLETE_FISCAL_YEARS]；600406[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]；601318[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]；603105[long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET、moat:DURABLE_MOAT_CORROBORATION_THRESHOLD_NOT_MET]
- 请求但未进入本次研究工件：**无**。
- 上一次完整终态 run：`35823336693`；执行 **SUCCESS**；研究终态 **EVIDENCE_EXHAUSTED**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`35823336693`；当前 Lambda：`35823336693`。
- 请求：**6**；研究 BUY：**0**；研究 WAIT_PRICE：**0**；研究 RESEARCH_GAP：**6**；研究 REJECT：**0**。
