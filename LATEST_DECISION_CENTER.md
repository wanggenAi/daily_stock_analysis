...
- 盘中价覆盖：**0/4**；交易时段：**ACTIVE_AFTERNOON**；行情状态：**DEGRADED**。
...
| Deep Provenance 证据审计 | **ACTIVE** | audit=True / run=36101480489 / unverified-pass=0 |
...
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`36101041992`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`36101041992`
...
- 请求但未进入本次研究工件：**无**。
- 上一次完整终态 run：`36101041992`；执行 **SUCCESS**；研究终态 **EVIDENCE_EXHAUSTED**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`36101041992`；当前 Lambda：`36101041992`。
...
- 当前可用判断：**17**；其中 ENTRY_NOW **1**；Jev run：`36101644731`。
