...
|---|---:|---:|---:|---|---|---|---|---|
| 国电南瑞 600406 | 22.05 | 17.46 | -4.65 | REDUCE_25 | **维持减仓25%目标；本轮无新增减仓/退出信号；目标减50股，当前可执行0股（手数约束；禁止向上取整）** | HIGH | ACTIVE/seen=177 | DEEP_REVIEW_PARTIAL |
| 润贝航科 001316 | 28.11 | 49.37 | 9.19 | HOLD_REVIEW | **持有观察** | LOW | ACTIVE/seen=179 | DEEP_REVIEW_PARTIAL |
| 中国平安 601318 | 53.37 | 83.07 | -4.64 | HOLD | **继续持有** | MEDIUM | ACTIVE/seen=104 | DEEP_REVIEW_PARTIAL |
| 洛阳钼业 603993 | 17.51 | 28.13 | -5.98 | HOLD | **继续持有；历史分批加仓授权已消费，本轮新增可执行0股** | HIGH | ACTIVE/seen=146 | DEEP_REVIEW_PARTIAL |

### 每只持仓的决策链

- **国电南瑞 600406**：现价 22.05 / 价值中枢 17.46（价/值 1.26；价值区间 12.84–26.09；区位 **UPPER_VALUE**）；估值信心 **HIGH**；Formal **REDUCE_25**；Deep **PASS 0 / FAIL 0 / UNKNOWN 5**；Lifecycle **ACTIVE / seen=177**；原因码：`V31_IMMEDIATE_VALUATION_SELL;REDUCE_25;SELL_RATIONALE_STABLE_VALUE_PRICE_OVEREXTENSION`。
- **润贝航科 001316**：现价 28.11 / 价值中枢 49.37（价/值 0.57；价值区间 21.64–70.29；区位 **FAIR_VALUE**）；估值信心 **LOW**；Formal **HOLD_REVIEW**；Deep **PASS 2 / FAIL 0 / UNKNOWN 3**；Lifecycle **ACTIVE / seen=179**；原因码：`VALUATION_CONFIDENCE_LOW;REALISTIC_GROWTH_UNSTABLE`。
- **中国平安 601318**：现价 53.37 / 价值中枢 83.07（价/值 0.64；价值区间 未形成完整区间；区位 **UNKNOWN**）；估值信心 **MEDIUM**；Formal **HOLD**；Deep **PASS 0 / FAIL 0 / UNKNOWN 5**；Lifecycle **ACTIVE / seen=104**；原因码：`INSURER_EVIDENCE_VALID;NO_ACTION_THRESHOLD;price_to_neutral=0.642<1.00`。
- **洛阳钼业 603993**：现价 17.51 / 价值中枢 28.13（价/值 0.62；价值区间 18.03–43.07；区位 **BELOW_VALUE**）；估值信心 **HIGH**；Formal **HOLD**；Deep **PASS 4 / FAIL 0 / UNKNOWN 1**；Lifecycle **ACTIVE / seen=146**；原因码：`FUNDAMENTALS_INTACT;NO_ACTION_THRESHOLD`。
...
| 能力 | 状态 | 当前真正产出的结果 |
|---|---|---|
| 市场大趋势 / 全A脉搏 | **ACTIVE** | GREEN / score=73.83 / 2026-09-18 |
...
| Candidate Lifecycle 持续研究记忆 | **ACTIVE** | active=118 / events=7749 / focus=10 |
| Deep 五类硬门槛 + 官方证据 | **ACTIVE** | requested=16 / verified-pass=218 / unresolved=69 |
| Deep Provenance 证据审计 | **ACTIVE** | audit=True / run=35617254124 / unverified-pass=0 |
...
- 国电南瑞 600406：ACTIVE / tier=PENDING / 历史被系统重新看见 177 次。
- 润贝航科 001316：ACTIVE / tier=PENDING / 历史被系统重新看见 179 次。
- 中国平安 601318：ACTIVE / tier=PENDING / 历史被系统重新看见 104 次。
- 洛阳钼业 603993：ACTIVE / tier=PENDING / 历史被系统重新看见 146 次。

## 自动深算运行状态

- 当前运行状态来源：**TERMINAL_STATUS**
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`35616396593`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`35616396593`
- 触发来源：`EVIDENCE_LAYER_CHANGE`
- 计算执行：**SUCCESS**
- 运行状态：**COMPLETED**
- 研究过程终态：**EVIDENCE_EXHAUSTED**
- 请求深算：**16**；已处理：**16**；完整：**0**；证据穷尽：**16**。
- Workset profile：总数 **853**；请求代码已落 profile **16**；handoff 未完成 **0**；覆盖可审计：**True**；完整覆盖：**True**。
- 同轮补证据尝试：**2**；取得证据：**38**；推进硬门槛：**1**。
- 尚未解决硬门槛：**69**。
...
- 请求但未进入本次研究工件：**无**。
- 上一次完整终态 run：`35616396593`；执行 **SUCCESS**；研究终态 **EVIDENCE_EXHAUSTED**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`35616396593`；当前 Lambda：`35616396593`。
