...
| Deep 五类硬门槛 + 官方证据 | **ACTIVE** | requested=2 / verified-pass=0 / unresolved=7 |
...
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`36046611587`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`36046611587`
- 触发来源：`JEV_ORCHESTRATOR_36045994064`
- 计算执行：**SUCCESS**
- 运行状态：**COMPLETED**
...
- 同轮补证据尝试：**2**；取得证据：**3**；推进硬门槛：**1**。
- 尚未解决硬门槛：**7**。
- 未决原因摘要：涉及 2 只；门槛分布：long_term_demand×2、moat×2、predictability×1、earnings_authenticity×1、financial_safety×1；Top原因：OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET×2、INSUFFICIENT_MULTI_YEAR_MOAT_EVIDENCE×2、INSUFFICIENT_CONSECUTIVE_COMPLETE_FISCAL_YEARS×1、SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT×1、SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT×1；样例：600095[long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET、moat:INSUFFICIENT_MULTI_YEAR_MOAT_EVIDENCE、predictability:INSUFFICIENT_CONSECUTIVE_COMPLETE_FISCAL_YEARS]；600827[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]
- 请求但未进入本次研究工件：**无**。
- 上一次完整终态 run：`36046611587`；执行 **SUCCESS**；研究终态 **EVIDENCE_EXHAUSTED**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`36046611587`；当前 Lambda：`36046611587`。
