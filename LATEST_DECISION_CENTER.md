...
| Deep 五类硬门槛 + 官方证据 | **ACTIVE** | requested=17 / verified-pass=210 / unresolved=63 |
| Deep Provenance 证据审计 | **ACTIVE** | audit=True / run=35959665987 / unverified-pass=0 |
...
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`35959237906`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`35959237906`
- 触发来源：`SLOW_LANE_TERMINAL_URGENT_REOPEN`
- 计算执行：**SUCCESS**
- 运行状态：**COMPLETED**
- 研究过程终态：**EVIDENCE_EXHAUSTED**
- 请求深算：**17**；已处理：**17**；完整：**1**；证据穷尽：**16**。
- Workset profile：总数 **500**；请求代码已落 profile **17**；handoff 未完成 **0**；覆盖可审计：**True**；完整覆盖：**True**。
- 同轮补证据尝试：**2**；取得证据：**41**；推进硬门槛：**5**。
- 尚未解决硬门槛：**63**。
...
- 请求但未进入本次研究工件：**无**。
- 上一次完整终态 run：`35959237906`；执行 **SUCCESS**；研究终态 **EVIDENCE_EXHAUSTED**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`35959237906`；当前 Lambda：`35959237906`。
...
- 当前可用判断：**12**；其中 ENTRY_NOW **1**；Jev run：`35959791793`。
