...
|---|---:|---:|---:|---|---|---|---|---|
| 国电南瑞 600406 | 22.23 | 17.46 | -3.87 | REDUCE_25 | **维持减仓25%目标；本轮无新增减仓/退出信号；目标减50股，当前可执行0股（手数约束；禁止向上取整）** | HIGH | ACTIVE/seen=182 | DEEP_REVIEW_PARTIAL |
| 润贝航科 001316 | 28.90 | 49.37 | 12.25 | HOLD_REVIEW | **持有观察** | LOW | ACTIVE/seen=184 | DEEP_REVIEW_PARTIAL |
| 中国平安 601318 | 54.46 | 83.07 | -2.69 | HOLD | **继续持有** | MEDIUM | ACTIVE/seen=109 | DEEP_REVIEW_PARTIAL |
| 洛阳钼业 603993 | 17.81 | 28.13 | -4.37 | HOLD | **继续持有；历史分批加仓授权已消费，本轮新增可执行0股** | HIGH | ACTIVE/seen=151 | DEEP_REVIEW_PARTIAL |

### 每只持仓的决策链

- **国电南瑞 600406**：现价 22.23 / 价值中枢 17.46（价/值 1.27；价值区间 12.84–26.09；区位 **UPPER_VALUE**）；估值信心 **HIGH**；Formal **REDUCE_25**；Deep **PASS 0 / FAIL 0 / UNKNOWN 5**；Lifecycle **ACTIVE / seen=182**；原因码：`V31_IMMEDIATE_VALUATION_SELL;REDUCE_25;SELL_RATIONALE_STABLE_VALUE_PRICE_OVEREXTENSION`。
- **润贝航科 001316**：现价 28.90 / 价值中枢 49.37（价/值 0.59；价值区间 21.64–70.29；区位 **FAIR_VALUE**）；估值信心 **LOW**；Formal **HOLD_REVIEW**；Deep **PASS 2 / FAIL 0 / UNKNOWN 3**；Lifecycle **ACTIVE / seen=184**；原因码：`VALUATION_CONFIDENCE_LOW;REALISTIC_GROWTH_UNSTABLE`。
- **中国平安 601318**：现价 54.46 / 价值中枢 83.07（价/值 0.66；价值区间 未形成完整区间；区位 **UNKNOWN**）；估值信心 **MEDIUM**；Formal **HOLD**；Deep **PASS 0 / FAIL 0 / UNKNOWN 5**；Lifecycle **ACTIVE / seen=109**；原因码：`INSURER_EVIDENCE_VALID;NO_ACTION_THRESHOLD;price_to_neutral=0.650<1.00`。
- **洛阳钼业 603993**：现价 17.81 / 价值中枢 28.13（价/值 0.63；价值区间 18.03–43.07；区位 **BELOW_VALUE**）；估值信心 **HIGH**；Formal **HOLD**；Deep **PASS 4 / FAIL 0 / UNKNOWN 1**；Lifecycle **ACTIVE / seen=151**；原因码：`FUNDAMENTALS_INTACT;NO_ACTION_THRESHOLD`。
...
| Deep 五类硬门槛 + 官方证据 | **ACTIVE** | requested=12 / verified-pass=223 / unresolved=34 |
| Deep Provenance 证据审计 | **ACTIVE** | audit=True / run=35698472835 / unverified-pass=0 |
...
- 深算资料来源：**AUTOMATIC_DEEP_CALCULATION**；资料 Lambda：`35698134033`；与当前运行一致：**True**
- 深算 profile lineage 与当前 runtime 一致。
- Lambda run：`35698134033`
- 触发来源：`JEV_ORCHESTRATOR_35697815789`
- 计算执行：**SUCCESS**
- 运行状态：**COMPLETED**
- 研究过程终态：**HANDOFF_INCOMPLETE**
- 请求深算：**12**；已处理：**8**；完整：**0**；证据穷尽：**8**。
- Workset profile：总数 **501**；请求代码已落 profile **8**；handoff 未完成 **4**；覆盖可审计：**True**；完整覆盖：**False**。
- 同轮补证据尝试：**2**；取得证据：**18**；推进硬门槛：**2**。
- 尚未解决硬门槛：**34**。
- 未决原因摘要：涉及 12 只；门槛分布：long_term_demand×8、moat×7、predictability×7、earnings_authenticity×6、financial_safety×6；Top原因：OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET×8、SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT×6、SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT×6、INSUFFICIENT_MULTI_YEAR_MOAT_EVIDENCE×6、INSUFFICIENT_CONSECUTIVE_COMPLETE_FISCAL_YEARS×6；样例：000096[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]；002042[earnings_authenticity:SAME_RUN_PIT_EARNINGS_AUTHENTICITY_EVIDENCE_INSUFFICIENT、financial_safety:SAME_RUN_PIT_FINANCIAL_SAFETY_EVIDENCE_INSUFFICIENT、long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET]；002282[profile:REQUESTED_CODE_NOT_PRESENT_IN_DEEP_PROFILE]；600283[profile:REQUESTED_CODE_NOT_PRESENT_IN_DEEP_PROFILE]；600309[long_term_demand:OFFICIAL_EVIDENCE_RETRY_EXHAUSTED_OR_CORROBORATION_NOT_MET、predictability:CYCLICAL_RESOURCE_REQUIRES_EXPLICIT_CYCLE_RESILIENCE_EVIDENCE]
- 请求但未进入本次研究工件：**002282、600283、603617、603739**。
- 上一次完整终态 run：`35698134033`；执行 **SUCCESS**；研究终态 **HANDOFF_INCOMPLETE**。
- 是否需要你手工开启下一轮：**False**。
...
- 终态来源 Lambda：`35698134033`；当前 Lambda：`35698134033`。
