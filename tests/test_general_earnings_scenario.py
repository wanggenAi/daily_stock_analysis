...
from src.strategies.genge_opportunity_discovery.selection_framework_v311 import decide_v311
...
def test_complete_evidence_builds_ordered_current_pv_scenarios():
    ...
    assert result["v31_extreme_stress_value"] <= result["v31_pessimistic_value"] <= result["v31_neutral_value"] <= result["v31_optimistic_value"]
    assert result["v31_potential_max_fundamental_loss_pct"] == result["v31_pessimistic_value"] / 20.0 - 1.0
...
def test_builder_does_not_supply_arbitrary_defaults_or_cross_model_routes():
    result = build_general_earnings_scenario(_row(valuation_primary_strategy_id="insurance_embedded_value"))
...
def test_current_pv_is_not_mislabelled_as_three_year_terminal_value():
    ...
    assert result["v31_general_scenario_risk_adjusted_cagr_status"] == "SCENARIO_PROBABILITIES_AND_TERMINAL_VALUES_REQUIRED"
...
def test_future_financial_observation_cannot_leak_into_current_inputs():
    ...
    panel = pd.DataFrame([
        {"report_date": "2026-06-30", "available_date": "2026-08-30", "normalized_eps_round6": 2.0, "realistic_growth_round6": 0.10, "neutral_value_round6": current_neutral, "normalized_earnings_observation_count": 4, "deduct_factor_round6": 0.9, "cash_conversion": 1.1, "realistic_growth_four_report_range": 0.03, "eps_growth_3y_round6": 0.10, "revenue_growth_3y_round6": 0.08},
        {"report_date": "2026-09-30", "available_date": "2026-10-30", "normalized_eps_round6": 99.0, "realistic_growth_round6": 0.30, "neutral_value_round6": future_neutral, "normalized_earnings_observation_count": 4, "deduct_factor_round6": 1.0, "cash_conversion": 2.0, "realistic_growth_four_report_range": 0.01, "eps_growth_3y_round6": 0.30, "revenue_growth_3y_round6": 0.30},
    ])
    result = current_inputs_from_panel("000001", panel, current_price=20.0, as_of=as_of, price_source="TEST", price_date=as_of)
...
def test_v311_decision_payload_carries_fresh_scenario_without_granting_buy():
    row = _row(
    ...
    payload = decide_v311(row).as_dict()
    assert payload["v31_general_scenario_status"] == "READY"
    assert payload["v31_scenario_valuation_ready"] is True
    assert payload["v31_pessimistic_value"] > 0
    assert payload["v31_optimistic_value"] >= payload["neutral_value"]
    assert "v31_risk_adjusted_3y_cagr" not in payload
