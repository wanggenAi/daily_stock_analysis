...
def test_observer_runs_only_after_successful_production_finalizer() -> None:
    ...
    assert 'test "$WORKFLOW" = "GenGe V3.1.1 Production Finalizer"' in text
...
def test_observer_validates_authoritative_chain_before_persistence() -> None:
    ...
    assert "validate_authority(authority, snapshot, hourly_view=hourly, daily_view=daily)" in text
