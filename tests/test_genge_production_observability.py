...
def _durable_supporting_state(root: Path, ts: str):
    _write(root / "data/evidence_events/index.json", {"generated_at": ts, "event_count": 8, "security_count": 4})
    _write(root / "data/evidence_events/collector_status.json", {"generated_at": ts})
    _write(root / "data/transactions/holdings_projection.json", {"holding_count": 4})
    _learning(root, ts)
...
def test_observability_detects_consistent_durable_state(tmp_path):
    ...
    _durable_supporting_state(tmp_path, ts)
    ...
    assert payload["canonical_source_run_ids_seen"] == [run]
    assert payload["research_learning"]["p0_count"] == 2
    assert payload["research_learning"]["parameter_tuning_allowed"] is False


def test_observability_allows_derived_state_to_retain_historical_identity(tmp_path):
    current_sid = "snap-current"; current_run = "run-current"
    old_sid = "snap-previous-material-change"; old_run = "run-previous-material-change"
    ts = "2026-08-28T06:00:00+00:00"
    _write(tmp_path / "data/production_status/latest.json", {"canonical_snapshot_id": current_sid, "canonical_source_run_id": current_run})
    _write(tmp_path / "data/hourly_research_state/latest.json", {"canonical_snapshot_id": current_sid, "canonical_source_run_id": current_run, "generated_at": ts, "workset_count": 4})
    _write(tmp_path / "data/hourly_deep_overlay/latest.json", {"canonical_snapshot_id": current_sid, "canonical_source_run_id": current_run, "generated_at": ts})
    _write(tmp_path / "data/opportunity_snapshots/candidate_lifecycle_state.json", {"latest_applied_snapshot_id": old_sid, "last_persisted_source_run_id": old_run, "candidates": {"600001": {}}})
    _write(tmp_path / "data/opportunity_snapshots/holding_valuation_continuity_state.json", {"latest_applied_snapshot_id": old_sid, "latest_applied_source_run_id": old_run})
    _durable_supporting_state(tmp_path, ts)

    payload = build(tmp_path, now=datetime(2026, 8, 28, 7, 0, tzinfo=timezone.utc))

    assert payload["health"] == "HEALTHY"
    assert payload["checks"]["canonical_snapshot_identity_consistent"] is True
    assert payload["checks"]["canonical_source_run_identity_consistent"] is True
    assert payload["canonical_snapshot_ids_seen"] == [current_sid]
    assert payload["canonical_source_run_ids_seen"] == [current_run]
    assert payload["derived_state_snapshot_ids_seen"] == [old_sid]
    assert payload["derived_state_source_run_ids_seen"] == [old_run]


def test_observability_degrades_on_active_identity_conflict(tmp_path):
    ts = "2026-08-28T06:00:00+00:00"
    _write(tmp_path / "data/production_status/latest.json", {"canonical_snapshot_id": "snap-a", "canonical_source_run_id": "run-a"})
    _write(tmp_path / "data/hourly_research_state/latest.json", {"canonical_snapshot_id": "snap-b", "canonical_source_run_id": "run-b", "generated_at": ts})
    _write(tmp_path / "data/hourly_deep_overlay/latest.json", {"canonical_snapshot_id": "snap-a", "canonical_source_run_id": "run-a", "generated_at": ts})
    ...
    assert "canonical_source_run_identity_consistent" in payload["failed_checks"]
