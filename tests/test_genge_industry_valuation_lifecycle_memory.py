...
def test_active_candidate_missing_from_today_all_a_is_materialized_for_research(
    ...
    with (report / "all_a_quant_screen.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(
            stream,
            fieldnames=[
                "code", "stock_name", "industry", "quant_status",
                "quant_rank", "quant_score", "hard_blockers",
            ],
        )
        writer.writeheader()
        writer.writerow(
    ...
    with (industry / "industry_top_candidates.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(
            stream,
            fieldnames=[
                "code", "stock_name", "industry", "industry_research_rank",
                "quant_status", "quant_score", "hard_blockers",
            ],
        )
        writer.writeheader()
        writer.writerow(
