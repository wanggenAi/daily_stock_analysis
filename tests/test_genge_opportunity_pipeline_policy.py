...
def test_strong_trend_removes_only_legacy_price_blockers(monkeypatch):
    ...
    row = {"price_percentile_5y": 0.82, "trend_confirmation_level": "STRONG"}
...
def test_price_flexibility_improves_research_proximity_but_not_tier_a(monkeypatch):
    monkeypatch.setattr(policy, "_ORIGINAL_TIER_ROW", _legacy_tier_stub)
    row = {
    ...
    assert result["v31_a_eligible"] is False
    assert "moat" in result["v31_hard_gate_unknowns"]


def test_explicit_frozen_v31_gates_can_authorize_tier_a(monkeypatch):
    monkeypatch.setattr(policy, "_ORIGINAL_TIER_ROW", _legacy_tier_stub)
    row = {
        "price_percentile_5y": 0.70,
        "trend_confirmation_level": "STRONG",
        ...
        "v31_candidate_class": "A3",
    }
    result = policy._tier_row(row)
    assert result["tier"] == "TIER_A"
    assert result["v31_a_eligible"] is True
    assert result["v31_candidate_class"] == "A3"
