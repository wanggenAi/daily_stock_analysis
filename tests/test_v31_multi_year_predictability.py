...
def test_quarterly_section_cannot_contaminate_annual_metrics():
    text = """
    七、 近三年主要会计数据和财务指标
    (一) 主要会计数据
    单位：元 币种：人民币
    主要会计数据 2025年 2024年 本期比上年同期增减(%) 2023年
    营业收入 4,735,621,058.43 4,374,948,942.73 8.24 4,408,052,320.27
    归属于上市公司股东的净利润 837,099,258.25 603,698,838.16 38.66 165,054,933.06
    经营活动产生的现金流量净额 1,087,737,899.26 1,072,673,062.78 1.40 1,785,770,737.45
    ...
    """
    metrics = extract_report_metrics(text, 2025)
    assert metrics["revenue"] == 4_735_621_058.43
    assert metrics["net_profit"] == 837_099_258.25
    assert metrics["operating_cash_flow"] == 1_087_737_899.26
    assert "1,064,399,637.59" not in metrics["metric_provenance"]["revenue"]["excerpt"]
    assert "195,348,364.84" not in metrics["metric_provenance"]["net_profit"]["excerpt"]
...
def test_quarterly_only_table_cannot_supply_annual_metrics():
    ...
    for metric in ("revenue", "net_profit", "operating_cash_flow"):
        ...
        assert provenance["verified"] is False
        assert provenance["reason"] == "QUARTERLY_METRIC_NOT_ANNUAL"
...
def test_multi_year_collector_uses_cninfo_when_primary_exchange_returns_clean_empty(monkeypatch):
    report_text = {
        2023: """
        单位：万元
        营业收入 10,000.00
    ...
    assert rows[0]["query_diagnostic"] == "PRIMARY_INSUFFICIENT_REPORTS:0"
