...
def test_predictability_sse_history_keeps_distinct_complete_years(monkeypatch):
    ...
    assert all((end - start).days <= predictability.SSE_MAX_QUERY_SPAN_DAYS for start, end in calls)
    assert all(next_start == end + predictability.timedelta(days=1) for (_, end), (next_start, _) in zip(calls, calls[1:]))
