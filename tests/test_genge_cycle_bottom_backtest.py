...
def test_balanced_double_close_stop_requires_second_confirm_for_medium_trend() -> None:
    signal = _signal_with_trend("2024-01-01", "MEDIUM", stop_loss=94.0)
    closes = [100.0] * 90
    price_df = _price_frame(closes)
    future_rows = price_df.iloc[1:].reset_index(drop=True)
    future_rows.loc[1, "low"] = 93.5
    future_rows.loc[1, "close"] = 93.8
    future_rows.loc[2, "low"] = 95.0
    future_rows.loc[2, "close"] = 96.0

    reclaimed_after_one_close = simulate_exit_policy(
        signal=signal,
        entry_price=100.0,
        future_rows=future_rows,
        horizon_days=60,
        ...
        params={
            ...
            "stop_loss_max_pct": 12.0,
            "stop_confirm_by_close": True,
            "stop_confirm_close_days": 2,
            "stop_confirm_min_trend_rank": 2,
            "stop_hard_intraday_pct": 2.5,
            "trail_start_pct": 18.0,
            "trail_drawdown_pct": 12.0,
            "profit_high_pct": 26.0,
    ...
    second_close_rows = future_rows.copy()
    second_close_rows.loc[2, "low"] = 93.0
    second_close_rows.loc[2, "close"] = 93.6
    confirmed = simulate_exit_policy(
        signal=signal,
        entry_price=100.0,
        future_rows=second_close_rows,
        horizon_days=60,
        stop_loss=94.0,
        policy_name=BALANCED_EXIT_POLICY_NAME,
        params={
    ...
    assert reclaimed_after_one_close[f"{BALANCED_EXIT_POLICY_NAME}_exit_reason_60d"] == "TIME_EXIT_60D"
    assert confirmed[f"{BALANCED_EXIT_POLICY_NAME}_exit_reason_60d"] == "STOP_LOSS"
    assert confirmed[f"{BALANCED_EXIT_POLICY_NAME}_exit_holding_days_60d"] == 3


def test_balanced_double_close_stop_does_not_delay_weak_or_hard_stop() -> None:
    weak_signal = _signal_with_trend("2024-01-01", "WEAK", stop_loss=94.0)
    medium_signal = _signal_with_trend("2024-01-01", "MEDIUM", stop_loss=94.0)
    closes = [100.0] * 90
    price_df = _price_frame(closes)
    future_rows = price_df.iloc[1:].reset_index(drop=True)
    future_rows.loc[1, "low"] = 93.0
    future_rows.loc[1, "close"] = 93.6
    params = {
        "stop_loss_min_pct": 6.0,
        "stop_loss_max_pct": 12.0,
        "stop_confirm_by_close": True,
    ...
    weak_result = simulate_exit_policy(
        signal=weak_signal,
        entry_price=100.0,
        future_rows=future_rows,
        horizon_days=60,
    ...
    hard_rows = future_rows.copy()
    hard_rows.loc[1, "low"] = 91.0
    hard_result = simulate_exit_policy(
    ...
    assert weak_result[f"{BALANCED_EXIT_POLICY_NAME}_exit_reason_60d"] == "STOP_LOSS"
    assert hard_result[f"{BALANCED_EXIT_POLICY_NAME}_exit_reason_60d"] == "STOP_LOSS"
