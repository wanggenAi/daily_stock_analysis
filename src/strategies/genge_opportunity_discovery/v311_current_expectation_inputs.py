...
def current_inputs_from_panel(
    ...
    *,
    ...
    return {
        ...
        if all(value is not None for value in (price, normalized, realistic, neutral))
        and np.isfinite(implied)
        else "HOLD_REVIEW_INPUT_INCOMPLETE",
        ...
        # Do not fabricate v31_expectation_gap_thesis. It remains qualitative.
        ...
        "v311_input_error": "" if implied_status != "INPUT_INCOMPLETE" else "IMPLIED_GROWTH_INPUT_INCOMPLETE",
    }
...
def fetch_latest_close(code: str, *, as_of: date) -> tuple[float | None, str]:
    import akshare as ak
    ...
    try:
        ...
        return float(local.sort_values(date_column).iloc[-1][close_column]), "AKSHARE_QFQ_DAILY"
    except Exception as exc:  # network/provider boundary
        return None, f"AKSHARE_QFQ_ERROR:{type(exc).__name__}"


def _source_price_map(rows: Iterable[Mapping[str, Any]]) -> dict[str, float]:
    fields = ("raw_latest_close", "v31_current_price", "current_price", "close", "latest_close")
