...
This sidecar deliberately performs only the part of insurance appraisal that is
fully observable: reviewed annual EV/NBV disclosures plus a point-in-time market
capitalization are used to reverse-solve the NBV franchise multiple currently
implied by the market.

No subjective franchise multiple is introduced, so this module does not publish
a synthetic fair value.  Ambiguous group/SOTP EV scopes fail closed.  Results
are research-only and never alter Formal BUY, ranking, position sizing or trade
execution.
"""
...
def execute_insurance_rows(
    ...
    *,
    ...
    for raw in rows:
        ...
        if strategy_id != INSURANCE_STRATEGY_ID:
            result.append(row)
            continue
        ...
        if not resolution.execution_eligible or resolution.input is None:
            status = (
                "INSURANCE_GROUP_EV_NBV_SCOPE_REQUIRED"
                if code == "601319"
                else f"DISCLOSED_EV_NBV_INPUTS_{resolution.status}"
            )
            row.update(
                {
                    ...
                    "insurance_model_next_action": (
                        "collect_scope_consistent_group_or_sotp_ev_nbv_without_double_counting"
                        if code == "601319"
        ...
        # Baidu historical total-market-cap values are in CNY 100m.  The
        # reviewed annual-report EV/NBV registry is in CNY million.
        ...
        row.update(
            {
                ...
                "insurance_current_p_ev": market_cap_cny_million / item.embedded_value,
                "insurance_implied_nbv_franchise_multiple": implied_multiple,
                "insurance_model_next_action": "review_negative_or_positive_market_implied_nbv_franchise_value_without_inventing_fair_multiple",
            }
        )
        result.append(row)
    return result
...
def write_insurance_execution_sidecar(
    ...
    *,
    ...
    output_path = report_dir / "insurance_valuation_execution.csv"
    _write_csv(output_path, executed, source_fields)
    ...
    summary = {
        ...
        "insurance_input_required_count": sum(
            row.get("insurance_model_execution_state") == "INSURANCE_MODEL_SELECTED_INPUTS_REQUIRED"
    ...
    lines = [
        ...
        f"- market_cap_input_basis: {MARKET_CAP_INPUT_BASIS}",
    ...
    for row in insurance_rows:
        lines.extend(
            [
                ...
                f"- execution: {row.get('insurance_model_execution_state')}",
                f"- status: {row.get('insurance_model_status')}",
                ...
                f"- market cap (CNYm): {row.get('insurance_market_cap_cny_million')}",
                ...
                f"- next: {row.get('insurance_model_next_action')}",
                "",
            ]
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--input-config",
        type=Path,
        default=Path("config/insurance_embedded_value_inputs.yaml"),
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
