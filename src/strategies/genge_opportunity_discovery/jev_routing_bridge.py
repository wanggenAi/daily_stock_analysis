...
def _choice(
    ...
    return choice, float(confidence) if isinstance(confidence, (int, float)) else None
...
def build_routing_bridge(shadow_payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for raw_row in shadow_payload.get("rows") or []:
        ...
        route, route_confidence = _choice(decisions, "research_route", ALLOWED_ROUTES)
        priority, priority_confidence = _choice(
            decisions, "attention_priority", ALLOWED_PRIORITIES
        )
        evidence_state, evidence_state_confidence = _choice(
            decisions, "evidence_state", ALLOWED_EVIDENCE_STATES
        )
