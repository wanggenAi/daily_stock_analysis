...
When a Jev-orchestrated Deep run has reached the exact matching Terminal, Investor Overlay, and Three-Pillar Decision Center state, the lineage reconciler now schedules one fresh **GenGe Jev Shadow Evaluation** for that Deep run. The workflow run is keyed by the exact Deep run id and checks for an existing matching workflow-dispatch run before creating another one, so repeated reconciler wake-ups reuse the same continuation instead of duplicating research.

The continuation returns through the normal boundary: Jev performs typed advisory classification/routing, the deterministic orchestrator reconciles the strategy ledger against the completed Deep lineage, and only a newly eligible strategy/evidence epoch may dispatch another bounded Deep workset. If the current gate-local epoch has no novel supported strategy, the ledger can settle that path as exhausted and the orchestrator skips it while continuing to consider later priority rows. The reconciler never directly relaunches Deep from this continuation step.

This closes the autonomous research-control loop without expanding trading authority: Jev remains advisory, deterministic eligibility remains authoritative, Formal BUY remains disabled, UNKNOWN remains distinct from PASS, and no-auto-trade remains true.
