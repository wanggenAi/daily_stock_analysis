...
系统允许某天没有任何买入或卖出信号，禁止为了“每天有票”放宽门槛。退出画像必须包含可追溯数据版本、当前规则版本、最近市场数据日期、至少 30 个总样本和至少 10 个最近两年样本；过期、版本不匹配或不可追溯时自动降级。全 A 生产入口会为当日 Top80 获取独立长历史前复权行情，按“当日只能看当日以前数据、下一交易日开盘进入、连续形态至少间隔 5 个交易日”的规则滚动回放 balanced exit policy，并覆盖候选的旧种子画像。旧画像不再因为刚好包含某只股票而永久通过，也不会因为候选与静态种子没有交集而永久失败。
...
板块差异化风控配置位于 `config/board_risk_rules.yaml`。每次运行还会生成股票池来源、排除原因、板块分布、双价格映射、候选升降级、证据变化、退出画像覆盖、`strict_gate_audit.csv` 和报告哈希清单。`run_summary.json.strict_gate_feasibility` 按每一道严格门槛给出失败数量，可以区分真实市场条件未通过和数据链路不可达。生产验收枚举为：
