...
- [新功能] GenGe 新增“硬逻辑 × 当前价格预期地图”：公司硬逻辑与市场/技术/入场/RR/仓位/退出等执行条件解耦，允许多个硬逻辑研究对象同时保留；公司通过后用当前 PE 相对严格历史参考 PE 反向估值，直接输出当前价格隐含利润增长、最高可接受买入价和深度低估价，预期已打满则等待更低价格，缺少未来增长证据时不编造预测。
- [测试] 新增硬逻辑价格地图回归与生产 workflow，覆盖多候选保留、执行门槛非否决、结构性硬风险、历史估值倒推、显式业务增长预期差、买入/深度低估价格上限、artifact 合并及 no-auto-trade 契约。
...
- [修复] 参考画像用每笔样本的真实退出执行日划分不重叠时期，缺失、越界或无法证明成交的结束日直接阻断；除时期收益、置信下界和回撤外，新增成员胜率、成员收益/回撤尾部以及单只股票最多贡献 50% 时期的约束，参考画像通过者仓位预算减半。
- [修复] 退出画像升级到 v8 可执行计划口径：支撑、阻力、ATR 和突破几何统一在 qfq 上计算，再用精确日期 factor 映射 raw 并按一分钱 tick 取整；raw OHLC 判断真实触发，避免除权前假高点和低价 qfq 两位小数误差。待入场或持有窗口跨公司行动时形成未完成审计样本并阻断画像，不允许贡献 `PASSED`。
- [修复] 历史/线上退出共用 balanced-v7 与冻结逻辑失效状态机；所有完整收盘信号按普通 A 股 T+1 在下一交易日开盘回放，纳入跳空、费用和滑点，最大回撤不读取卖出后的日内低点。一字板不伪造买入成交；退出遇一字跌停时保持待执行并继续计算风险，直到首个可执行开盘，数据截止时仍未解锁则阻断画像。第 60 日退出至少需要第 61 根开盘证据，样本间隔按真实退出执行日控制。
- [修复] 退出画像回放消除日内未来信息和亏损样本淘汰：突破改为收盘放量确认后下一交易日开盘成交，确认前或确认日先触发冻结止损/失效位会取消；完整历史先计算逐日 MA20/MA60，再截取退出窗口，确保第 45 日后的趋势退出与线上一致。画像日期取真实行情截止日，数据哈希覆盖 qfq/raw OHLC、精确因子、成交量、金额、入场参数和规则版本。长历史增加双数据源及带 SHA256/日期校验的 7 日故障回退缓存，候选或参考覆盖率低于 75% 时禁止新买入但仍生成研究报告。
- [修复] 正式突破信号改为两阶段生命周期；所有固定入场计划最多等待 10 个已观察交易日，并冻结计划创建日的 raw/qfq 基准。可能持仓改为按入场日读取长历史保留的精确 factor，避免前复权最新价比率仍为 1 时漏掉拆股/送股重写；基准变化或缺失优先于旧 raw 止损并转人工复核。每日信号 CSV 同步输出两类基准、当前复核值和该行情是否成功推进状态。同一行情重复运行保持幂等；同日数据更正、状态缺失、损坏或版本不兼容会关闭新买入，运行断档则按“期间可能成交”转持仓确认，若冻结阈值已击穿直接给出条件退出，不能静默假定未成交。
- [新功能] 观察到参考入场后，以同一前复权口径逐日推进 balanced-v7 退出状态，覆盖保护止损、原计划逻辑失效、确认后的趋势破坏、盈利回撤、长期未修复和第 60 个参考持有交易日退出；同日入场/退出条件会保存参考状态并明确下一交易日最早退出。每日信号新增最早退出日与执行时点，所有 `HOLD_REVIEW`/`SELL_EXIT` 仍要求用户确认真实成交与持仓，不连接券商。
- [修复] 活跃入场/可能持仓信号不再受 Top80/Top30 截断，新增 `active_signal_review_queue.csv`；Top5 与每日观察信号展示完整严格阻断条件和个股/参考退出画像统计，非正式买入候选按安全阻断、失败门槛族、失败总数和可操作性依次排序；汇总新增 `exit_profile_strategy_health`，把真实无历史优势与任务失败明确分开。
- [改进] 全 A 退出画像新增 Top80 外每周最多 40 只的行业分散探索队列；此前有效的通过画像优先保留，其余只按研究层级、ISO 周、行业和代码哈希轮换，并输出 `exit_profile_exploration_queue.csv`。通过画像会强制纳入基本面抓取和深度证据流程，不受正常 Top80/基本面名额截断，但所有画像和现实风险门槛保持不变。
- [测试] 新增固定参考分区、目标逐股剔除、真实执行期不重叠、成员尾部和集中度、长历史精确 factor/raw 映射及当日缓存校验、qfq 缩放不变性、公司行动窗口阻断、可观察历史门槛、交易日连续性、突破次日成交/确认前失效、完整历史均线、T+1 同日止损/次日跳空、一字板入场拒绝与退出延迟、逻辑失效 parity、live balanced-v7 状态/幂等、运行断档可能成交、真实前复权重写、同日行情更正、状态损坏及 GitHub 产物契约回归测试。
- [新功能] 全 A 生产扫描新增大盘、行业、个股量价和近 730 日官方重大事件风险层；巨潮/上交所公告支持多事件识别、活动/全部解除/部分解除/过期生命周期，扫描不完整时按未知风险阻断正式条件买入。
- [新功能] 全 A 每日固定输出五只 `daily_candidate_top5` 研究候选；Top5 保留买入、持仓复核、退出、取消或观察的真实动作，零仓位观察项与正式 `BUY_IF_TRIGGERED` 可执行清单严格分离。
- [修复] 买卖信号新增入场生命周期：未观察到参考入场时不再误报 `SELL_EXIT`，退出沿用冻结计划并推进已验证的 balanced-v7 状态，且要求确认真实持仓；同一股票不会同时进入退出与可买清单。
...
- [修复] 信号生命周期冻结原始价格计划，同日重跑保持幂等；交易日运行断档不再臆造“期间未成交”，待入场信号会转为需要确认的可能持仓，冻结退出阈值已被击穿时给出条件 `SELL_EXIT`，否则保持零仓位人工复核。
...
- [修复] 全 A 退出画像不再只依赖 76 只静态种子；每日研究候选、活跃信号和固定参考分区使用长历史前复权行情执行无未来数据泄漏、结果窗口不重叠的 balanced exit 滚动回放，并以当日结果覆盖候选旧画像，消除候选与静态合格样本无交集时严格买入永远不可达的问题。
...
- [新功能] 沪深全 A 生产扫描新增可审计的每日 `BUY_IF_TRIGGERED`、`HOLD_REVIEW`、`SELL_EXIT`、`CANCEL_BUY_REVIEW`、`WATCH_ONLY` 信号及独立买入/退出 CSV；只有严格候选允许条件买入，退出信号跟踪冻结止损、逻辑失效位和 balanced-v7 状态，但不读取持仓。
